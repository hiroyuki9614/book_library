...
2. Confirm the target database and `BOOK_FILE_STORAGE_ROOT` are the intended pair and that a current backup and restore evidence exist.
...
Attach the migration name and checksum, preflight review, database backup identifier/checksum, protected-file inventory/checksum, fileHash verification output, start/end times, operator, approver, and smoke results. A failed frontend or backend gate preserves Draft status; it does not authorize migration.