...
2. Confirm the candidate digest, prior approved digest, migration history, and compatibility decision. Confirm the database and active-storage evidence share a consistency boundary.
...
4. If incompatible, do not run an ad hoc down migration. Obtain approval for a forward fix, or restore PostgreSQL and the required active-storage state/evidence into isolated resources, validate them, and obtain explicit recovery approval before any environment action.
...
Do not reset the database, use `prisma db push`, invent a down migration, restore only PostgreSQL without validating active-storage references, delete files to make records fit, replace files, perform R2 lifecycle cleanup, or claim recovery from a database-only result. Logical/permanent deletion and storage reconciliation remain outside release packaging.
