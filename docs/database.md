...
認証処理には Better Auth を使用する。パスワードはユーザーテーブルへ直接保持せず、
認証プロバイダー情報とともにアカウントテーブルで管理する。

### セッションテーブル

Better Auth が発行するログインセッションを管理する。
セッショントークン、有効期限、接続元情報をユーザーに紐づけて保持する。

### アカウントテーブル

Better Auth の認証プロバイダー情報を管理する。
メールアドレス・パスワード認証のハッシュ化済みパスワードもこのテーブルで保持する。

### 検証トークンテーブル

メールアドレス確認やパスワードリセットなどで使用する一時的な検証情報を管理する。
...
Cloudflare R2 のファイルURL、拡張子、MIMEタイプ、元ファイル名、保存ファイル名、ファイルサイズを管理する。
