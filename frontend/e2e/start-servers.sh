...
echo "[e2e] checking DATABASE_URL"
echo "$DATABASE_URL" | grep -E "e2e|test" >/dev/null || {
  echo "ERROR: DATABASE_URL does not look like an E2E/test database: $DATABASE_URL"
  exit 2
}
...
cd ../backend
INITIAL_ADMIN_EMAIL="$INITIAL_ADMIN_EMAIL" INITIAL_ADMIN_NAME="$INITIAL_ADMIN_NAME" INITIAL_ADMIN_PASSWORD="$INITIAL_ADMIN_PASSWORD" npm run create:initial-admin || {
  echo "ERROR: create:initial-admin failed"
  exit 3
}
